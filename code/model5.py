################################################
# using 1 class: 9 _ standing up / 33 _ 시계보기(?)
################################################
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(self.num_img):
            ...
            img.save("./generated_image/model5/generated_img_%03d_%d.png" % (epoch, i))  # 이어서 학습
...
# plt.show()
plt.savefig('./history/model5/train_history.png')

# '''
# save model
np.save('./history/model5/PREDICTIONS', PREDICTIONS)

    # 1. HDF5
try:
    # gan.save('./history/model5/model5', save_format="tf")
    gan.save('./history/model5/model5.h5')
    print("전체 모델 저장_tf??")
...
with open("./history/model5/model5.json", "w") as json_file:
    json_file.write(gan_json)
print("전체 모델 저장_json")


gan.save_weights('./history/model5/model5_weight.h5')
# gan.save_weights('./history/model5_weight')  # 뭔차이?
...
#
# try:
#     plot_model(gan, show_shapes=True, show_layer_names=True, to_file='./history/model5/model5.png')
# except Exception as e:
#     print(e)

'''
# load model
with open("./history/model5/model5.json", "r") as model_json:
...
loaded_model.load_weights("./history/model5/model5_weight.h5")
...
# save model
np.save('./history/model5/PREDICTIONS(2)', PREDICTIONS)

    # 1. HDF5
try:
    loaded_model.save('./history/model5/model5(2).h5')
    print("전체 모델 저장_tf??")
...
with open("./history/model5/model5(2).json", "w") as json_file:
    json_file.write(gan_json)
print("전체 모델 저장_json")


loaded_model.save_weights('./history/model5/model5_weight(2).h5')
# '''
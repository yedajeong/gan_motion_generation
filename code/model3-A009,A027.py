###############################################
# using 2 classes: 9, 27 _ standing up, jump up
###############################################
...
print(data.shape)  # (_, 128, 25, 3)
print(passCnt)

np.save(reposDir+'/data_class9', data)
'''

# dataset load
reposDir = '/Users/ydj89/Desktop/다도미/2022-2 소융캡스톤디자인/repos'
train_images = np.load(reposDir + '/data_class9.npy')
train_images = np.concatenate([train_images, np.load(reposDir + '/data_class27.npy')], 0)
# train_images = data
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(self.num_img):
            ...
            img.save("./generated_image/model3-A009/generated_img_%03d_%d.png" % (epoch, i))  # 이어서 학습
...
# plt.show()
plt.savefig('./history/model3-A009/train_history.png')

# '''
# save model
np.save('./history/model3-A009/PREDICTIONS', PREDICTIONS)

    # 1. HDF5
try:
    # gan.save('./history/model3-A009/model3-A009', save_format="tf")
    gan.save('./history/model3-A009/model3-A009.h5')
    print("전체 모델 저장_tf??")
...
with open("./history/model3-A009/model3-A009.json", "w") as json_file:
    json_file.write(gan_json)
print("전체 모델 저장_json")


gan.save_weights('./history/model3-A009/model3-A009_weight.h5')
# gan.save_weights('./history/model3-A009_weight')  # 뭔차이?
...
#
# try:
#     plot_model(gan, show_shapes=True, show_layer_names=True, to_file='./history/model3-A009/model3-A009.png')
# except Exception as e:
#     print(e)

'''
# load model
with open("./history/model3-A009/model3-A009.json", "r") as model_json:
...
loaded_model.load_weights("./history/model3-A009/model3-A009_weight.h5")
...
# save model
np.save('./history/model3-A009/PREDICTIONS(2)', PREDICTIONS)

    # 1. HDF5
try:
    loaded_model.save('./history/model3-A009/model3-A009(2).h5')
    print("전체 모델 저장_tf??")
...
with open("./history/model3-A009/model3-A009(2).json", "w") as json_file:
    json_file.write(gan_json)
print("전체 모델 저장_json")


loaded_model.save_weights('./history/model3-A009/model3-A009_weight(2).h5')
# '''

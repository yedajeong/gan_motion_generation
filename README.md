...
- (왼쪽에서 오른쪽으로) (a)원점 이동 전 학습 data (b)grid artifact 제거 전 생성 이미지 (c)grid artifact 제거 후 생성 이미지 (a)로 학습한 이미지 (d)원점 이동 후 학습 data (e)(d)로 학습한 이미지

- (a)생성된 fake image (b) (a)를 모션패치화 (c)연속 동작의 일부 frame 캡쳐
